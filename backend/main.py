...
from predict_text import predict_department_from_text
from image_predict import predict_image, preprocess_image, department_mapping
...
class ReportCreate(BaseModel):
    ...
    def validate_urgency_level(cls, v):
        ...
        if v not in valid_urgency_levels:
            raise ValueError(
                f"Urgency level must be one of: {', '.join(valid_urgency_levels)}"
            )
        return v
...
async def initialize_database(db: AsyncSession = Depends(get_db)):
    try:
        ...
        categories = [
            Category(
                name="Infrastructure", description="Roads, bridges, public facilities"
            ),
        ...
        result = await db.execute(
            select(User).filter(User.email == "admin@urbanissues.com")
        )
...
async def create_report(report_data: ReportCreate, db: AsyncSession = Depends(get_db)):
    try:
        ...
        if not reported_status:
            raise HTTPException(
                status_code=500, detail="Reported status not found in database"
            )
...
async def signup(user_data: UserCreateEnhanced, db: AsyncSession = Depends(get_db)):
    ...
    result = await db.execute(
        select(User).filter(User.mobile_number == user_data.mobile_number)
    )
    ...
    if login_data.email in MOCK_USERS:
        ...
        if login_data.password == mock_user["password"]:
            access_token = create_access_token(
                data={"sub": mock_user["email"], "is_admin": mock_user["is_admin"]}
            )
            return {
...
async def get_todays_activity(db: AsyncSession = Depends(get_db)):
    ...
    resolved_reports_result = await db.execute(
        ...
        .where(Status.name == "Resolved", func.date(Report.updated_at) == today)
...
async def get_issue_confirmations(report_id: int, db: AsyncSession = Depends(get_db)):
    """
...
async def get_dashboard_stats(db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        resolved_reports_result = await db.execute(
            select(func.count(Report.id)).join(Status).filter(Status.name == "Resolved")
        )
        ...
        return {
            ...
            "resolution_rate": round(
                (resolved_reports / total_reports * 100) if total_reports > 0 else 0, 1
            ),
            "category_stats": [
                {"category": cat, "count": cnt} for cat, cnt in category_stats
            ],
            "urgency_stats": [
                {"urgency": urg, "count": cnt} for urg, cnt in urgency_stats
            ],
        }
...
async def get_category_summary(db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        return {
            "category_summary": [
                {"category_name": name, "description": desc, "issue_count": count}
        ...
        for r in reports:
            ...
            if r.category_id:
                cat_res = await db.execute(
                    select(Category).filter(Category.id == r.category_id)
                )
            ...
            if r.status_id:
                stat_res = await db.execute(
                    select(Status).filter(Status.id == r.status_id)
                )
            ...
            if status_filter == "active" and status_name not in [
                "Reported",
                "In Progress",
            ]:
...
async def search_user_reports(
    ...
    user_email: str = Query(
        ..., description="User email to search reports"
    ),  # CHANGED BACK: Made mandatory
    db: AsyncSession = Depends(get_db),
):
    """
...
async def get_report_timeline(report_id: int, db: AsyncSession = Depends(get_db)):
    """
    ...
    try:
        ...
        if report_status and report_status.name in [
            "In Progress",
            "Resolved",
            "Closed",
        ]:
            ...
            timeline_events.append(
                {
                    ...
                    "status": "completed"
                    if report_status.name in ["Resolved", "Closed"]
                    else "in_progress",
                }
            )
...
async def resolve_issue(
    ...
    try:
        ...
        if not resolved_status:
            raise HTTPException(
                status_code=500, detail="Resolved status not found in Status table"
            )
    ...
    try:
        ...
        for dept_key, dept_name in department_display_names.items():
            ...
            total_result = await db.execute(
                select(func.count(Report.id)).where(Report.department == dept_key)
            )
            ...
            efficiency = round(
                (resolved / total_issues * 100) if total_issues > 0 else 0, 1
            )
...
async def get_resolution_trends(
    months: int = Query(6, ge=1, le=12), db: AsyncSession = Depends(get_db)
):
    """
    ...
    try:
        ...
        for dept_key, dept_name in department_map.items():
            ...
            result = await db.execute(query, {"dept": dept_key, "months": months})
            ...
            for row in rows:
                efficiency = (row.resolved / row.total) * 100 if row.total > 0 else 0
                data.append(round(efficiency, 1))
                labels.append(row.month.strftime("%b"))

            trends.append({"department": dept_name, "data": data, "months": labels})

        return {"trends": trends}

    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Resolution trend error: {str(e)}")
        ...
        return {
            ...
            "breakdown": {
                ...
                "pending_percentage": round(
                    (pending / total_issues * 100) if total_issues > 0 else 0, 1
                ),
                "progress_percentage": round(
                    (progress / total_issues * 100) if total_issues > 0 else 0, 1
                ),
            },
        }
...
async def get_issues_by_department(
    ...
    try:
        ...
        for dept_key, dept_name in department_display_names.items():
            ...
            if count > 0:
                data.append({"department": dept_name, "issues_count": float(count)})
...
async def update_issues_status(
    ...
    try:
        ...
        for issue_id in update.issue_ids:
            result = await db.execute(select(Report).filter(Report.id == issue_id))
            report = result.scalar_one_or_none()
            if report:
...
async def debug_check_user_reports(
    ...
    try:
        ...
        exact_result = await db.execute(
            select(Report).filter(Report.user_email == user_email)
        )
        exact_reports = exact_result.scalars().all()

        # Also check if there are any similar emails
        similar_result = await db.execute(
            select(Report.user_email, func.count(Report.id)).group_by(Report.user_email)
        )
        ...
        return {
            ...
            "all_emails_in_database": [
                {"email": email, "count": count} for email, count in all_emails
            ],
...
async def get_map_issues(
    ...
    try:
        ...
        stmt = select(Report).where(
            Report.location_lat.isnot(None), Report.location_long.isnot(None)
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Error fetching map issues: {str(e)}"
        )
...
async def get_issues_in_bounds(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error fetching bounded issues: {str(e)}"
        )
...
async def get_map_stats(db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error fetching map stats: {str(e)}"
        )
...
async def get_monthly_trends(db: AsyncSession = Depends(get_db)):
    ...
    for i in range(5, -1, -1):
        ...
        monthly_data.append(
            {"month": start.strftime("%b"), "issues": result.scalar() or 0}
        )
...
async def predict_department(
    ...
    try:
        ...
        return {
            ...
            "image_prediction": {"department": img_pred, "confidence": img_conf}
            if img_pred
            else None,
            "success": True,
        }
...
async def predict_image_only(file: UploadFile = File(...)):
    """Endpoint for image-only prediction used by floating wizard"""
    try:
        ...
        raise HTTPException(500, f"Image prediction error: {str(e)}")
        ...
        for issue in issues:
            try:
                ...
                if (
                    ...
                    and prediction_response.get("department") != "other"
                    and prediction_response.get("confidence", 0) > 50
                    ...
                    print(
                        f"✅ Assigned issue {issue.id} to {issue.department} "
...
async def get_assignment_status(db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        auto_assigned_result = await db.execute(
            select(func.count(Report.id)).where(Report.auto_assigned == True)
        )
        ...
        unassigned_result = await db.execute(
            select(func.count(Report.id)).where(Report.department == "other")
        )
        ...
        confidence_result = await db.execute(
            select(func.avg(Report.prediction_confidence)).where(
                Report.auto_assigned == True
            )
        )
        ...
        return {
            ...
            "auto_assigned_percentage": round(
                (auto_assigned_count / total_issues * 100) if total_issues > 0 else 0, 1
            ),
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get assignment status: {str(e)}"
        )
...
async def get_auto_assigned_issues(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get auto-assigned issues: {str(e)}"
        )
...
async def get_user_dashboard_stats(
    ...
    total = await db.scalar(
        select(func.count(Report.id)).where(Report.user_email == user_email)
    )

    resolved = await db.scalar(
        select(func.count(Report.id))
        .join(Status)
        .where(Report.user_email == user_email, Status.name == "Resolved")
    )

    today_reports = await db.scalar(
        select(func.count(Report.id)).where(
            Report.user_email == user_email, func.date(Report.created_at) == today
        )
    )

    week_reports = await db.scalar(
        select(func.count(Report.id)).where(
            Report.user_email == user_email, func.date(Report.created_at) >= week_start
        )
    )
...
async def get_citizen_trust_score(
    ...
    # Total reports
    total_result = await db.execute(
        select(func.count(Report.id)).filter(Report.user_email == current_user.email)
    )
    total = total_result.scalar() or 0
    ...
    resolved_result = await db.execute(
        select(func.count(Report.id))
        .join(Status)
        .filter(Report.user_email == current_user.email, Status.name == "Resolved")
    )
    resolved = resolved_result.scalar() or 0
